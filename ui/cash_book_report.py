...
class CashBookReportUI:
    """Cash Book report screen (Expenzo voucher data)."""
    ...
    def _render(self, report: Dict[str, Any]) -> None:
        ...
        self.status.set(
            f"Cash Book generated: {report.get('transaction_count', 0)} transactions "
    ...
    def _export_to_png(self) -> None:
        ...
        success, path = report_exporter.export_table_to_png(self.table, "cash_book")
        if success:
    ...
    def _export_to_csv(self) -> None:
        if not self.current_report_data:
            dialogs.warn("Export", "Generate the report first.", parent=self.parent)
            return
        success, path = cash_book_service.export_cash_book_to_csv(self.current_report_data, "cash_book")
        if success:
            dialogs.info("Export", f"Exported to:\n{path}", parent=self.parent)
        else:
    ...
    def _export_to_json(self) -> None:
        if not self.current_report_data:
            dialogs.warn("Export", "Generate the report first.", parent=self.parent)
            return
        success, path = cash_book_service.export_to_json(self.current_report_data, "cash_book")
        if success:
            dialogs.info("Export", f"Exported to:\n{path}", parent=self.parent)
        else:
            dialogs.error("Export", path, parent=self.parent)
